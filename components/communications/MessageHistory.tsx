...
const MessageHistory: React.FC<Props> = ({ messages }) => {
    if (messages.length === 0) {
        return (
            <Card className="border-none shadow-soft rounded-2xl p-12">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {messages.map((message) => (
                            <tr key={message.id} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-center">
                                    <span
                                        className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${message.status === 'SENT'
