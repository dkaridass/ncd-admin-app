    ...
    return (
        <div className="bg-white rounded-[3rem] shadow-premium overflow-hidden border border-slate-100">
            <div className="bg-slate-50 p-6 flex justify-between items-center border-b border-slate-100">
                <h3 className="text-xl font-black text-primary font-display uppercase italic">{monthLabel}</h3>
                ...
                {days.map((dayObj, idx) => {
                    ...
                    return (
                        <div
                            ...
                            className={`min-h-[120px] p-2 border-b border-r border-slate-50 transition-colors group relative ${dayObj.isCurrentMonth ? 'bg-white hover:bg-slate-50' : 'bg-slate-50/30'}`}
                        >
                            <div className="flex justify-between items-start mb-2">
                                <span className={`text-xs font-bold ${isToday ? 'bg-primary text-white w-6 h-6 rounded-full flex items-center justify-center' : dayObj.isCurrentMonth ? 'text-slate-700' : 'text-slate-300'}`}>
                                    {dayObj.date.getDate()}
                                </span>
                                {dayObj.isCurrentMonth && (
                                    <button className="opacity-0 group-hover:opacity-100 text-slate-300 hover:text-primary transition-opacity">
                                ...
                                {dayEvents.slice(0, 3).map(event => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`px-2 py-1 rounded-lg text-[9px] font-bold truncate cursor-pointer transition-all hover:scale-[1.02] ${isSpecial ? 'bg-amber-100 text-amber-800 border border-amber-200 ring-1 ring-amber-400/30' :
                                                    isCulte ? 'bg-indigo-50 text-indigo-700 border border-indigo-100' :
