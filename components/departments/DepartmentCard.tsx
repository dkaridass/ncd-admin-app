...
const DepartmentCard: React.FC<DepartmentCardProps> = ({ department, onClick, isMeetingToday }) => {
    ...
    return (
        <Card
            ...
            className="group relative overflow-hidden cursor-pointer hover:shadow-lg transition-all border border-slate-100 hover:border-primary/20 bg-white rounded-[2rem] p-8"
        >
            {/* Status Dot */}
            ...
            <div className="flex items-center gap-4 mb-6">
                <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center text-xs font-black text-slate-400 border border-slate-100">
                    #{department.number}
                </div>
                <div>
                    <h3 className="text-lg font-display font-black text-primary uppercase italic tracking-tight leading-none group-hover:text-secondary transition-colors line-clamp-1">
                        {department.name}
                    </h3>
            ...
            <div className="space-y-3 mb-6 bg-slate-50/50 p-4 rounded-2xl border border-slate-50">
                <div className="flex justify-between items-center">
                    ...
                    <span className="text-[10px] font-bold text-primary truncate max-w-[120px]">{displayLeaderName}</span>
                </div>
                {displayVpName && (
                    <div className="flex justify-between items-center">
                        <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Adjoint</span>
                        <span className="text-[10px] font-bold text-slate-500 truncate max-w-[120px]">{displayVpName}</span>
                    </div>
                )}
            ...
            <div className="flex gap-2">
                ...
                <Button variant="ghost" className="flex-1 text-[9px] font-black uppercase tracking-widest bg-slate-50 text-slate-400 group-hover:bg-secondary/10 group-hover:text-secondary transition-colors">
                    Ouvrir le Hub
                </Button>
            </div>
        </Card>
