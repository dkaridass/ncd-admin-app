...
const DepartmentDashboard: React.FC<DepartmentDashboardProps> = ({ isOpen, onClose, department }) => {
    ...
    const handleSubmitReport = async () => {
        ...
        // Firebase Storage mode (kept for later)
        ...
        try {
            ...
            const storagePath = `departmentReports/${department.id}/${year}/${safeFileName}`;
            ...
            const newReport: Omit<DepartmentReport, 'id'> = {
                ...
                year,
                submittedAt: new Date().toISOString().split('T')[0],
                content: `Rapport mensuel : ${selectedFile.name}`,
    ...
    const handleDeleteDepartment = async () => {
        ...
        let warningMessage = "⚠️ Attention: La suppression est irréversible.\n\n";
        if (hasMembers) {
            warningMessage += `• ${deptMembers.length} membre(s) assigné(s) à ce département\n`;
        }
        if (hasReports) {
            warningMessage += `• ${deptReports.length} rapport(s) associé(s) à ce département\n`;
        }
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Hub: ${department.name}`}>
            <div className="flex flex-col h-[80vh]">
                {/* Tabs */}
                <div className="flex gap-2 px-1 mb-6 border-b border-slate-100">
                    <button
                        onClick={() => setActiveTab('overview')}
                        className={`px-6 py-3 text-[10px] font-black uppercase tracking-widest border-b-2 transition-colors ${activeTab === 'overview' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                    >
                        Vue d'ensemble
                    </button>
                    <button
                        onClick={() => setActiveTab('members')}
                        className={`px-6 py-3 text-[10px] font-black uppercase tracking-widest border-b-2 transition-colors ${activeTab === 'members' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                    >
                        Membres <span className="ml-2 bg-slate-100 px-1.5 py-0.5 rounded text-[9px]">{deptMembers.length}</span>
                    </button>
                    <button
                        onClick={() => setActiveTab('reports')}
                        className={`px-6 py-3 text-[10px] font-black uppercase tracking-widest border-b-2 transition-colors ${activeTab === 'reports' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                    >
                        Rapports
                    </button>
                    {hasPermission('MANAGE_DEPARTMENTS') && (
                        <button
                            ...
                            className={`px-6 py-3 text-[10px] font-black uppercase tracking-widest border-b-2 transition-colors ${activeTab === 'settings' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                        >
                            Paramètres
                        </button>
                    )}
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar px-1 pb-4 space-y-8">
                        ...
                        <div className="space-y-8 animate-fade-in">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                                    ...
                                    <p className="text-xl font-black text-primary">{actualMemberCount}</p>
                                </div>
                                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                                ...
                                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                                    ...
                                    <p className="text-xl font-black text-primary">{departmentReports.filter(r => r.departmentId === department.id).length}</p>
                                </div>
                            </div>

                            {/* Leadership Section */}
                            <div className="bg-white p-6 rounded-[2rem] border border-indigo-50 shadow-sm relative overflow-hidden">
                                    ...
                                    {department.leaders && department.leaders.length > 0 ? (
                                        department.leaders.map((leader, index) => (
                                            <div key={index} className="flex items-center gap-4">
                                                <div className={`w-12 h-12 rounded-full flex items-center justify-center border-4 border-white shadow-sm ${leader.role === 'Titulaire' || leader.role === 'Pasteur' ? 'bg-indigo-100 text-indigo-500' : 'bg-slate-50 text-slate-400'}`}>
                                        ...
                                        <>
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm font-bold text-slate-800">{leader?.name || department.leaderName || 'Non assigné'}</p>
                                                </div>
                                            </div>
                                            ...
                                            {(vp || department.vpName) && (
                                                <div className="flex items-center gap-4">
                                                    <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center border-4 border-white shadow-sm">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-bold text-slate-800">{vp?.name || department.vpName}</p>
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            {(secondVp || department.secondVpName) && (
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-bold text-slate-800">{secondVp?.name || department.secondVpName}</p>
                                                    </div>
                                                </div>
                                            )}
                                        </>
                            ...
                            {isLoading && deptMembers.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {deptMembers.map(member => (
                                        <div key={member.id} className="flex items-center gap-4 p-4 bg-white border border-slate-100 rounded-2xl hover:shadow-sm hover:border-primary/20 transition-all">
                                            <img
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex gap-2 flex-wrap">
                                                    ...
                                                    {member.isLeader && <span className="text-[9px] font-black text-indigo-500 uppercase bg-indigo-50 px-2 rounded-full">Leader</span>}
                                                    {member.churchFunction && <span className="text-[9px] font-bold text-primary uppercase bg-primary/10 px-2 rounded-full">{member.churchFunction}</span>}
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                    ...
                    {activeTab === 'reports' && (
                        <div className="space-y-8 animate-fade-in">
                            <div className="bg-primary/5 p-8 rounded-[2.5rem] border-2 border-dashed border-primary/20 text-center relative overflow-hidden">
                                <h4 className="text-[10px] font-black uppercase text-primary tracking-[0.3em] mb-6">Soumettre Nouveau Rapport</h4>
                                {reportsMode === 'link' ? (
                                        ...
                                        <div className="bg-white rounded-[2rem] border-2 border-slate-100 p-6">
                                            ...
                                            <div className="mt-3">
                                                ...
                                                <textarea
                                                    ...
                                                    rows={3}
                                                    className="block w-full px-4 py-3 border-2 border-slate-200 rounded-xl bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-xs md:text-sm transition-all shadow-sm"
                                    ...
                                    <>
                                        ...
                                        {!selectedFile ? (
                                            <div
                                                ...
                                                className="cursor-pointer py-8 px-6 bg-white rounded-[2rem] border-2 border-slate-100 hover:border-secondary transition-all group shadow-sm"
                                            >
                                                <div className="w-12 h-12 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-3 group-hover:bg-secondary/10 transition-colors">
                                            ...
                                            <div className="py-6 px-6 bg-secondary/10 rounded-[2rem] border-2 border-secondary/30">
                                                <p className="text-[11px] font-black text-primary mb-4">{selectedFile.name}</p>
                                                <div className="flex gap-2 justify-center">
                                                    <button onClick={() => setSelectedFile(null)} className="px-4 py-2 bg-white text-red-500 rounded-xl text-[10px] font-black uppercase">Annuler</button>
                                                    <Button
                                                        onClick={handleSubmitReport}
                                                        disabled={isSubmittingReport}
                                                        isLoading={isSubmittingReport}
                            ...
                            <div className="space-y-4">
                                {departmentReports.filter(r => r.departmentId === department.id).length > 0 ? (
                                    departmentReports
                                        ...
                                        .map(report => (
                                            <div key={report.id} className="bg-white p-4 rounded-2xl border border-slate-100 flex justify-between items-center hover:shadow-sm transition-all">
                                                <div className="flex items-center gap-4 flex-1 min-w-0">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <p className="text-xs font-black text-primary uppercase truncate">{report.month} {report.year}</p>
                                                        {report.fileName && (
                                                            <p className="text-[9px] text-slate-400 truncate mt-1">{report.fileName}</p>
                                                        )}
                                                        <Badge variant={report.status === 'Approuvé' ? 'success' : report.status === 'Révisé' ? 'warning' : 'outline'} className="text-[8px] px-2 py-0.5 mt-1">{report.status}</Badge>
                                                    </div>
                                                </div>
                            ...
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full px-4 py-3 rounded-xl border border-slate-200 font-bold text-slate-700 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none"
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        rows={3}
                                        className="w-full px-4 py-3 rounded-xl border border-slate-200 font-bold text-slate-700 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none resize-none"
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
                                        {daysOfWeek.map(day => (
                                            <button
                                                ...
                                                disabled={isSavingSettings}
                                                className={`px-3 py-2 rounded-lg text-xs font-bold transition-all ${editMeetingDays.includes(day)
                                ...
                                <div className="border-t border-red-100 mt-8 pt-8">
                                    ...
                                    <div className="bg-red-50 p-4 rounded-xl mb-4">
                                        <p className="text-xs text-slate-600 mb-2 font-bold">⚠️ La suppression est irréversible.</p>
                                        {deptMembers.length > 0 && (
                                            <p className="text-xs text-slate-500">• {deptMembers.length} membre(s) assigné(s)</p>
                                        )}
                                        {departmentReports.filter(r => r.departmentId === department.id).length > 0 && (
                                            <p className="text-xs text-slate-500">• {departmentReports.filter(r => r.departmentId === department.id).length} rapport(s) associé(s)</p>
                                        )}
                                    </div>
                                    <Button
                                        onClick={handleDeleteDepartment}
                                        disabled={isDeleting}
                                        className="bg-red-50 text-red-500 hover:bg-red-500 hover:text-white rounded-xl px-6 py-3 uppercase font-black tracking-widest text-[10px] disabled:opacity-50"
                                    >
