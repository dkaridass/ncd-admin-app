      ...
      <div className="flex items-center gap-3 md:gap-6 mt-1">
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 truncate max-w-[120px] md:max-w-none">
              {syncStatus === 'syncing' ? 'Synchro...' : 'Connecté • Lubumbashi'}
            </span>
          </div>
          <div className="mt-0.5 hidden sm:block">
            <span className="text-[8px] font-black uppercase tracking-[0.2em] text-secondary dark:text-gold">Thème 2026 : Focus sur Jésus</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 md:gap-4">
        <div className="flex items-center px-2 md:px-4 py-2 rounded-lg hover:bg-slate-50/80 dark:hover:bg-card dark:bg-card-dark transition-all cursor-pointer group">
          ...
          <div className="ml-3 hidden md:block text-left">
            <p className="text-sm font-black text-primary dark:text-white leading-none font-display uppercase tracking-tight">{currentUser?.name}</p>
            <p className="text-[10px] text-slate-400 dark:text-gold/70 font-black uppercase mt-1 tracking-[0.2em]">{currentUser?.role}</p>
          </div>
        </div>
