...
const MembersPage: React.FC = () => {
    const { members, addMember, addMembersBatch, updateMember, deleteMember, language, isLoading } = useData();
    ...
    return (
        <PageTransition>
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-end mb-10 gap-6">
                <div>
                    <h2 className="text-4xl md:text-5xl font-extrabold text-primary dark:text-white font-display tracking-tight leading-none mb-4 uppercase italic">Répertoire</h2>
                    <div className="flex gap-4 md:gap-8 border-b border-slate-100 dark:border-dark lg:border-none">
                        <button onClick={() => setActiveTab('Fidèle')} className={`text-[10px] md:text-[11px] font-black uppercase tracking-widest pb-3 border-b-4 transition-all ${activeTab === 'Fidèle' ? 'border-secondary text-primary dark:text-white' : 'border-transparent text-slate-300 hover:text-slate-400'}`}>
                            {language === 'ln' ? 'BASALELI' : 'Fidèles'}
                        </button>
                        <button onClick={() => setActiveTab('Visiteur')} className={`text-[10px] md:text-[11px] font-black uppercase tracking-widest pb-3 border-b-4 transition-all ${activeTab === 'Visiteur' ? 'border-secondary text-primary dark:text-white' : 'border-transparent text-slate-300 hover:text-slate-400'}`}>
                            {language === 'ln' ? 'BAPAYA' : 'Identification'}
                        </button>
                        <button onClick={() => setActiveTab('Archivé')} className={`text-[10px] md:text-[11px] font-black uppercase tracking-widest pb-3 border-b-4 transition-all ${activeTab === 'Archivé' ? 'border-secondary text-primary dark:text-white' : 'border-transparent text-slate-300 hover:text-slate-400'}`}>
                            Archives
                        </button>
                    </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto items-center">
                    {/* Desktop Filters */}
                    <div className="hidden md:flex gap-2 bg-card dark:bg-card-dark rounded-full p-1 shadow-sm dark:shadow-none border border-slate-100 dark:border-dark">
                        {['Tous', 'Homme', 'Femme'].map(g => (
                            <button
                                key={g}
                                onClick={() => setFilterGender(g as any)}
                                className={`px-4 py-2 rounded-full text-[9px] font-black uppercase transition-all ${filterGender === g ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600' : 'text-slate-400 hover:text-slate-600 dark:text-slate-400'}`}
                            >
                                {g}
                            </button>
                        ))}
                    ...
                    <div className="flex gap-2 w-full lg:w-auto">
                        <div className="relative flex-1 sm:w-64">
                            <input
                                ...
                                className="w-full pl-6 pr-6 py-3 rounded-full border border-slate-200 dark:border-dark bg-card dark:bg-card-dark text-xs focus:ring-4 focus:ring-primary/5 focus:border-primary/20 font-bold transition-all outline-none"
                            />
                        </div>
                        ...
                        <button
                            ...
                            className="md:hidden flex items-center justify-center w-12 h-12 rounded-full bg-card dark:bg-card-dark border border-slate-200 dark:border-dark text-slate-500 hover:text-primary transition-colors flex-shrink-0"
                        >
                ...
                isFilterModalOpen && (
                    <div className="fixed inset-0 z-[100] flex items-end justify-center md:hidden">
                        ...
                        <div className="bg-card dark:bg-card-dark rounded-t-[2rem] w-full p-6 relative z-10 animate-fade-in border-t border-slate-100 dark:border-dark shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
                            ...
                            <h3 className="text-xl font-bold font-display text-primary dark:text-white mb-6">Filtres</h3>

                            <div className="space-y-6 mb-8">
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 block">Genre</label>
            ...
            <CsvImportModal
                ...
                onImport={async (membersToImport: Partial<Member>[]) => {
                    await addMembersBatch(membersToImport as Omit<Member, 'id'>[]);
                }}
            />
        </PageTransition >
    );
